...
##### ---- Phage data ---- #####
phage <- read.csv("./Experiment 2/original_data/reldiv_exp2_master.csv", skip=1) %>% 
  select(Diversity, Phage, Timepoint, Replicate, PFU) %>% 
  na.exclude

phage$Diversity %<>% as.factor
phage$Timepoint %<>% as.factor
phage$Replicate %<>% as.factor

# Relevel the datasets so things make sense
phage$Diversity %<>% relevel(ref="48")
phage$Diversity %<>% relevel(ref="24")
phage$Diversity %<>% relevel(ref="12")
phage$Diversity %<>% relevel(ref="6")
phage$Diversity %<>% relevel(ref="3")

#### -- Phage titre plot-- #####
pal <- wes_palette("Moonrise2", 2)

phage_plot <- ggplot(aes(y=log10(PFU+1), x=Timepoint), data=phage)+
  
  geom_boxplot(width=0.5, outlier.shape = NA, position=pd, aes(colour=Phage))+
  geom_point(width=0.2, alpha=0.5, pch=21, position=pd, aes(colour=Phage))+
  ...
  scale_y_continuous(breaks=c(seq(0,12,2)),
                     labels=pfu_labels)+
  scale_colour_manual(name = "Phage diversity",
...
ggsave("phage_comparison.png", phage_comparison, path="Experiment 2/figs/", device="png", dpi=600,
...
CRISPR_rate_comp <- ggplot(aes(x=Treatment, y=r), 
  ...
  geom_boxplot(width=0.5, position=pd, outlier.shape = NA, aes(colour=Experiment))+
  geom_point(alpha=0.5, pch=21, position=pd, aes(colour=Experiment))+
  ...
  scale_colour_manual(labels = c("CRISPR allele diversity", "CRISPR allele & phage\ndiversity"),
                      values=pal)+
  theme_cowplot()+
...
BIM_rate_comp <- ggplot(aes(x=Treatment, y=r), 
  ...
  geom_boxplot(width=0.5, position=pd, outlier.shape = NA, aes(colour=Experiment))+
  geom_point(position=pd, alpha=0.5, pch=21, aes(colour=Experiment))+
  geom_hline(yintercept=0, linetype=2, size=1)+
  # stat_summary(fun.y="mean", geom="point", aes(colour=Experiment), position=pd)+
  # stat_summary(fun.data = "mean_cl_boot", geom="errorbar", width=0, aes(colour=Experiment), position=pd)+
  facet_wrap(~Timepoint, labeller = timepoint_labeller, ncol=2)+
